...
public:
  ...
  static inline char* longest_filename() { 
    ...
    {
      // преобразуем к UTF 8
            uint16_t *plongFileName = (uint16_t*)longFilename;
            uint8_t c= 0;
            uint8_t size = 0 ;
            char buf[2], tb[20];
            SERIAL_ECHOLN("new string");
            while(*plongFileName != 0)
            {
               
                sprintf(tb, "byte %u",*plongFileName);
                SERIAL_ECHO(tb);
                SERIAL_ECHO(" ");
                size = UTF16toUTF8( *plongFileName ,buf);
                SERIAL_ECHO(buf);
                SERIAL_ECHO(" size ");
                SERIAL_ECHOLN(size);
                for(uint8_t i=0;i<size;i++)
                {
                  longFilename[c+i] = buf[i];
                }
                c+=size;
                plongFileName++;
            }
