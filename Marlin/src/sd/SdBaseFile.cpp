    ...
    if (longFilename && DIR_IS_LONG_NAME(dir)) {
      ...
      if (VFAT->firstClusterLow == 0) {
        ...
        if (WITHIN(seq, 1, MAX_VFAT_ENTRIES)) {
          ...
          n = (seq - 1) * (FILENAME_LENGTH)*2;
          
          memcpy(longFilename+n,VFAT->name1,10);
          memcpy(longFilename+n+10,VFAT->name2,12);
          memcpy(longFilename+n+22,VFAT->name3,4);
          //LOOP_L_N(i, FILENAME_LENGTH)
          /*for (uint8_t i=0 ; i<FILENAME_LENGTH ; i++) //dva byte
          {
            if(i < 10)
              longFilename[n + i] = (char)VFAT->name1[i];
              //longFilename[n + i+1] = (char)((char*)VFAT->name1+i+1);
            else if(i < 22)
              longFilename[n + i] = (char)VFAT->name2[i-10];
              else
              longFilename[n + i] = (char)VFAT->name3[i-22];
              
            //((uint16_t*)longFilename)[n + i] = (i < 5) ? VFAT->name1[i] : (i < 11) ? VFAT->name2[i - 5] : VFAT->name3[i - 11];
            //((uint16_t*)
          }
          */
          ...
          {
            //n = (seq - 1) * (FILENAME_LENGTH*2);
            uint8_t c = FILENAME_LENGTH-1;
            uint16_t *plongf = (uint16_t*)longFilename;
            plongf +=n;
            while(plongf[c] == 0xFFFF )
            {
              plongf[c] = 0x0000;
              c--;
            }
